# Claude Code Project Instructions — tungstenite-kotlin

## Project Overview

This is **tungstenite-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tungstenite`](https://crates.io/crates/tungstenite).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tungstenite/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tungstenite>
