...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Tungstenite.swiftmodule/`
...
//      `import Tungstenite` above would fail at compile time.
//
//   2. The static archive `libTungstenite.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Tungstenite" }` and
//      `flattenPackage = "io.github.kotlinmania.tungstenite"` configuration in
...
//      via the `TungsteniteLibrary` product.
//
...
final class TungsteniteExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Tungstenite swift module imported cleanly")
    }
}
